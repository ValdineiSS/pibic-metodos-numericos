...
def abrir_arquivo():
	arquivo = open("../Dados/Problema2-2-Método-Euler-Melhorado.txt", "w")
...
def plotar_grafico(coordenadas_x, coordenadas_y):
	...
	plt.savefig('../Gráficos/Problema-2-2-Euler-Melhorado.png', format='png', dpi=300)
	plt.show()
...
def main():
	...
	#Limite de execução definido para quando x (tempo) atinge 100
	while(x <= 100):
		count += 1
		...
		#Derivadas
		k1 = edo(S)
		k2 = edo(metodo_euler(S, h, edo(S)))

		#Atualizações
		S = metodo_euler_melhorado(S, h, k1, k2)
		x = x + h
