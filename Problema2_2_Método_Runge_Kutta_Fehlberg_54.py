...
def abrir_arquivo():
    arquivo = open("../Dados/Problema2-2-Método-Runge-Kutta-Fehlberg-54.txt", "w")
...
def plotar_grafico(coordenadas_x, coordenadas_y):
    ...
    plt.savefig('../Gráficos/Problema-2-2-Runge-Kutta-Fehlberg-54.png', format='png', dpi=300)
    plt.show()
...
#Limite de execução definido para quando x <= 100
    while(x <= 100):
        count += 1
        #pra não ultrapassar
        if 100 - x > 0:
