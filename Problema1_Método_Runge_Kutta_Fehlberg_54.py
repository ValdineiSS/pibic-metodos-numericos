...
'''Algoritmo que implementa o Método Numérico de Runge-Kutta-Fehlberg 54 na equação diferencial que modela 
...
def abrir_arquivo():
    arquivo = open("Problema1-Método-Runge-Kutta-Fehlberg-54.txt", "w")
...
#Condições Iniciais
    Q = 0
    #constantes de runge-kutta Fehlberg
    epsilon = 10**(-7) #Verificação empírica
    h = 0.5
    x = 0
    count = 0
...
#Escrever coordenadas no arquivo:
        arquivo.write("{: <30} {: <30} {: <30} {: <30} \n".format(str(count), str(x), str(Q), str(h)))
...
#Derivadas
        k1 = h * edo(Q)
        k2 = h * edo(Q + (k1/4))
        k3 = h * edo(Q + (3/32)*k1 + (9/32)*k2)
        k4 = h * edo(Q + (1932/2197)*k1 - (7200/2197)*k2 + (7296/2197)*k3)
        k5 = h * edo(Q + (439/216)*k1 - (8)*k2 + (3680/513)*k3 - (845/4104)*k4)
        k6 = h * edo(Q - (8/27)*k1 + (2)*k2 - (3544/2565)*k3 + (1859/4104)*k4 - (11/40)*k5)
